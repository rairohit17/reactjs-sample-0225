  ...
  return (
     <div className="px-[10px] overflow-hidden  overflow-y-auto no-scrollbar h-full ">
       ...
       <div className='flex gap-5 flex-wrap w-full'>
               {completeTasks.map((task)=><Card id={task.id as string} todo={task.todo} description={task.description} status={task.status} type={task.type}  ></Card>)}
             </div>
     </div>
   );
 }