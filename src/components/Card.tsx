  ...
  return (
    <div className="sm:w-[250px] sm:h-[250px] h-[180px] w-[150px] relative bg-slate-900 border  rounded-xl shadow-md">
      ...
      <CiStar
        ...
        className={` ${isImportant ? 'hidden' : 'inline-block'} cursor-pointer absolute bottom-3 right-4 sm:text-2xl`}
      />
      <FaStar
        onClick={() => setIsImportant(false)}
        className={`${isImportant ? 'inline-block' : 'hidden'} cursor-pointer absolute bottom-3 right-4 sm:text-2xl`}
      ></FaStar>
    </div>
  );
}
